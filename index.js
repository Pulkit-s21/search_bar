// if we use data-attr in querySelector we need to put them in []
...
fetch("https://jsonplaceholder.typicode.com/users").then(res => res.json()).then(data => {
    ...
    // here what we are saying is the card will return a document fragment and then copy the content of the userTemplate in the card and then clone everything inside it and then get the first child in that which is the card div

    // used for loop cz we want to do this process for every user in the API
    // data.forEach(user => {
    ...
    // setting user to whtever we return here and to return we need to use map so thats why we chngd from forEach to map
